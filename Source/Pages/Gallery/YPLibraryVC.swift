...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    public func selectedMedia(photoCallback: @escaping (_ photo: YPMediaPhoto) -> Void,
      ...
      DispatchQueue.global(qos: .userInitiated).async {
        ...
        if self.multipleSelectionEnabled && self.selection.count > 1 {
          ...
          self.loadAssets(index: 0, selectedAssets: selectedAssets, resultMediaItems: [], multipleItemsCallback: multipleItemsCallback)
        } else {
    ...
    func loadAssets(index: Int, selectedAssets: [(asset: PHAsset, cropRect: CGRect?)], resultMediaItems: [YPMediaItem], multipleItemsCallback: @escaping (_ items: [YPMediaItem]) -> Void) {
        var resultMediaItems = resultMediaItems
        if index < selectedAssets.count {
            let asset = selectedAssets[index]
            let index = index + 1
            switch asset.asset.mediaType {
            case .image:
                fetchImageAndCrop(for: asset.asset, withCropRect: asset.cropRect) { image, exifMeta in
                    let photo = YPMediaPhoto(image: image.resizedImageIfNeeded(), exifMeta: exifMeta, asset: asset.asset)
                    resultMediaItems.append(YPMediaItem.photo(p: photo))
                    self.loadAssets(index: index, selectedAssets: selectedAssets, resultMediaItems: resultMediaItems, multipleItemsCallback: multipleItemsCallback)
                }
            case .video:
                checkVideoLengthAndCrop(for: asset.asset, withCropRect: asset.cropRect) { videoURL in
